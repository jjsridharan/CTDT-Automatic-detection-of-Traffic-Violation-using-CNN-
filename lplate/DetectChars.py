...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("G:\\CTDT\lplate\\classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("G:\\CTDT\\lplate\\flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate,cordinate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
